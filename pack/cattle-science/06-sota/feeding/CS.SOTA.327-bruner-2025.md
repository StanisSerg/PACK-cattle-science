...
1. **Микроинкапсуляция кормовых добавок.** Morey et al. (2011) и Vittorazzi et al. (2022) описали микроинкапсуляцию как метод защиты питательных веществ и добавок с целью улучшения стабильности, палатабельности и целенаправленного высвобождения в пищеварительном тракте. Применительно к анионным солям исследования ограничены.

2. **Палатабельность и инкапсуляция.** Hultquist & Casper (2016) показали, что инкапсуляция карбоната калия со свободными жирными кислотами не вызывает проблем с палатабельностью у дойных коров. Coupland & Hayes (2014) обсуждали физическую инкапсуляцию как способ маскировки горького вкуса в фармацевтике.

3. **Личность коровы и кормовое поведение.** Meagher et al. (2017) и Neave et al. (2018) продемонстрировали связь между чертами личности (смелость, активность, исследовательское поведение) и вариабельностью кормового поведения. Более смелые коровы склонны пробовать новые корма и менее реактивны на новизну (Meagher et al., 2017).
...
### [СКРИНШОТ] Figure 1: Diagram of the experimental setup consisting of a cow in a stall with a fitted split-bunk feed area

**Название в статье:** Figure 1. Diagram of the experimental setup consisting of a cow in a stall with a fitted split-bunk feed area.
**Источник:** Bruner et al., 2025, стр. 5.
**Тип:** схема / диаграмма.

**Описание:**
Схематическое изображение экспериментальной установки: корова в индивидуальном стойле с разделённой кормушкой (split-bunk). Левая и правая секции предназначены для одновременного предъявления двух различных рационов (ANIC и PLCB). Схема иллюстрирует геометрию кормового места и механизм раздельного доступа.

**Ключевые элементы для лекции:**
- Разделённая кормушка — ключевой элемент дизайна предпочтения.
- Размеры секций и механизм предотвращения смешивания рационов.
- Применимость дизайна для других парных сравнений палатабельности.

**Комментарий лектора:**
> "Обратите внимание на конструкцию разделённой кормушки. Это позволяет корове выбирать между двумя рационами без физического смешивания, что критично для валидности теста предпочтения."

---

### [СКРИНШОТ] Table 2: Particle size distribution and nutrient content by particle size of the base dry cow diet and treatment diets

**Название в статье:** Table 2. Particle size distribution (mean ± SD) and nutrient content (mean ± SD) by particle size of the base dry cow diet and treatment diets.
**Источник:** Bruner et al., 2025, стр. 6.
**Тип:** таблица.

**Описание:**
Таблица содержит распределение частиц по фракциям (long >19 мм, medium 8–19 мм, short 4–8 мм, fine <4 мм) и химический состав каждой фракции (% СВ, ADF, NDF, CP, крахмал, жир, зола, Ca, P, K, Mg, Na, S, Cl, DCAD) для базового рациона, ANIC и PLCB. Ключевое наблюдение: анионная соль и жир-плацебо концентрируются преимущественно в fine-фракции (жир ANIC: 6,99% vs. 2,52% в базовом рационе; DCAD fine-фракции ANIC: −242,5 мЭк/кг).

**Ключевые элементы для лекции:**
- Fine-фракция содержит наибольшую концентрацию анионной соли и жира.
- Сортировка против fine-частей напрямую снижает поступление анионов.
- Объяснение расхождения между расчётным DCAD (−101,5 мЭк/кг) и фактическим (−22,4 мЭк/кг) — дилюция за счёт высокого ППК и сортировки.

**Комментарий лектора:**
> "Видите, fine-фракция несёт основную нагрузку анионов. Если корова сортирует против мелких частиц — она автоматически недополучает DCAD. Это объясняет, почему фактический DCAD был −22, а не −101 мЭк/кг."

---

### [СКРИНШОТ] Table 4: Effect of dry cow dietary treatment on intake, feed sorting, rumination, and urine pH during the baseline and treatment period

**Название в статье:** Table 4. Effect of dry cow dietary treatment on intake, feed sorting, rumination, and urine pH during the baseline and treatment period.
**Источник:** Bruner et al., 2025, стр. 8.
**Тип:** таблица.

**Описание:**
Результаты фазы приемлемости (acceptability). Основные переменные: ППК (кг/сут СВ), сортировка (%), руминация (мин/сут), pH мочи. ANIC не повлиял на ППК (19,2 vs. 19,5 кг/сут; P = 0,37) и руминацию (521,3 vs. 532,4 мин/сут; P = 0,41). Достоверное снижение pH мочи (7,6 vs. 8,2; P < 0,001). Сортировка: коровы сортировали против long-частиц PLCB (97,2%; P = 0,03), но не сортировали long-частицы ANIC (100,7%). В обеих обработках сортировка за medium (P < 0,05) и против short/fine (P < 0,05).

**Ключевые элементы для лекции:**
- ППК не снижается при использовании ANIC — ключевой практический вывод.
- pH мочи подтверждает биологическую активность продукта.
- Сортировка против long-частиц PLCB — артефакт текстуры рациона, не связанный с анионами.

**Комментарий лектора:**
> "Главное сообщение: микроинкапсуляция работает. Корова ест столько же, но моча кислее — значит, ацидоз запущен. Это win-win для transition-менеджмента."

---

### [СКРИНШОТ] Table 5: Effect of dry cow dietary treatment on proportion of intake, feed sorting, rumination, and urine pH during the 3-d preference test

**Название в статье:** Table 5. Effect of dry cow dietary treatment on proportion of intake, feed sorting, rumination, and urine pH during the 3-d preference test.
**Источник:** Bruner et al., 2025, стр. 9.
**Тип:** таблица.

**Описание:**
Результаты 3-дневного теста предпочтения. Соотношение потребления ANIC/PLCB = 48,6% / 51,4% (P = 0,26) — отсутствие предпочтения. Сортировка: в PLCB коровы сортировали против long-частиц (82,5%; P = 0,05 vs. ANIC). Тенденция к различиям в сортировке short-частиц (P = 0,07). Руминация и pH мочи не различались между рационами в тесте предпочтения (P > 0,59).

**Ключевые элементы для лекции:**
- При свободном выборе коровы не дискриминируют ANIC.
- Сортировка в PLCB-рационе связана с текстурой, не с вкусом анионов.
- Отсутствие различий в pH мочи в тесте предпочтения объясняется ad libitum потреблением обоих рационов и низкой общей кислотностью.

---

### [СКРИНШОТ] Table 6: Coefficients (loadings) of the eigenvectors for the factors extracted from each principal components analysis

**Название в статье:** Table 6. Coefficients (loadings) of the eigenvectors for the factors extracted from each principal components analysis of behaviors recorded when cows (n = 24) were subjected to a combined arena test.
**Источник:** Bruner et al., 2025, стр. 9.
**Тип:** таблица.

**Описание:**
Результаты PCA поведенческих тестов. Выделено 2 фактора: Factor 1 «Active» (собственное значение 2,08; 41,5% дисперсии; высокие нагрузки: walking arena 0,765, walking object 0,786, vigilance object 0,860) и Factor 2 «Bold» (собственное значение 1,35; 26,9% дисперсии; высокие нагрузки: time in contact with novel human 0,856, latency to contact −0,814). Sampling adequacy = 0,54.

**Ключевые элементы для лекции:**
- Два фактора объясняют 68,4% вариабельности поведения.
- «Bold» — готовность к контакту с незнакомцем; «Active» — общая двигательная активность.
- Интерпретация факторов важна для понимания индивидуальной вариабельности реакции на кормовые интервенции.

---

### [СКРИНШОТ] Figure 2: Association between principal component analysis trait, bold, and proportion of ANIC diet consumed

**Название в статье:** Figure 2. Association between principal component analysis trait, bold, of the personality test with the proportion of ANIC diet consumed while being offered PLCB diet simultaneously during the 3-d preference test (n = 24 cows).
**Источник:** Bruner et al., 2025, стр. 10.
**Тип:** scatter plot с линией регрессии.

**Описание:**
Scatter plot: ось X — Factor 2 (bold) PCA coordinates; ось Y — proportion of ANIC diet consumed (% of total DMI). Уравнение регрессии: y = −4,00x + 50,39; R² = 0,17. Отрицательная связь: менее смелые коровы (низкие значения bold) потребляли большую долю ANIC. Разброс точек значительный, объяснительная сила умеренная.

**Ключевые элементы для лекции:**
- Отрицательный наклон: смелость ↔ снижение предпочтения ANIC.
- R² = 0,17 — личность объясняет лишь часть вариабельности.
- Практическая импликация: в группе могут быть «аномальные» индивидуумы, требующие индивидуального подхода.

**Комментарий лектора:**
> "Смелые коровы — консерваторы. Они склонны пробовать оба рациона поровну. Менее смелые застревают на первом попавшемся — и если это ANIC, они едят его больше. Это не значит, что ANIC вкуснее; это артефакт поведенческой инерции."

---

### [СКРИНШОТ] Figure 3: Association between principal component analysis trait, active, and sorting of (a) medium, (b) short, and (c) fine particles

**Название в статье:** Figure 3. Association between principal component analysis trait, active, and sorting (%) of (a) medium (<19, >8 mm) particles, (b) short (<8, >4 mm) particles, and (c) fine (<4 mm) particles during the 3-d preference test (n = 24 cows).
**Источник:** Bruner et al., 2025, стр. 11.
**Тип:** три scatter plot с линиями регрессии (панели a, b, c).

**Описание:**
Панель a (medium): y = −2,21x + 106,14; R² = 0,21. Менее активные коровы сильнее сортировали за medium-частицами.
Панель b (short): y = 2,34x + 99,92; R² = 0,22. Более активные коровы сильнее сортировали за short-частицами.
Панель c (fine): y = 3,16x + 86,81; R² = 0,12. Более активные коровы тенденциально сильнее сортировали за fine-частицами (P = 0,07).

**Ключевые элементы для лекции:**
- Активность модифицирует паттерн сортировки, но не направленность предпочтения рациона.
- Менее активные коровы могут лучше получать рацион со средними частицами (где концентрируется энергия).
- Практическое следствие: мониторинг индивидуальной сортировки может быть полезен для precision feeding.
...
15. NASEM (2021). Nutrient Requirements of Dairy Cattle: Eighth Revised Edition. *National Academies Press*. [foundational reference, не цитируется в Bruner et., 2025]
...
- [x] Заполнение YAML frontmatter (v1.1)
...
- [ ] Post-creation checklist (scripts)
- [ ] Git commit
...
| 2026-05-17 | Написание SoTA v1.1 | Файл CS.SOTA.327-bruner-2025.md создан | 120 мин |
