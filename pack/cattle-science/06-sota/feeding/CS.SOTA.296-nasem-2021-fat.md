...
Коэффициенты переваримости по всему пищеварительному тракту варьируются от 0,31 (C16:0/C18:0 >90%) до 0,76 (Ca-соли пальмовых ЖК). NRC 2001 использовал единый подход, что приводило к систематическим ошибкам (NASEM 2021, p. 42).

**Доказательства:** Мета-регрессия Daley et al. (2020) на данных 30 исследований.

**Уверенность:** 0,90 (мета-регрессия Daley et al., 2020, n = 30; R² = 0,68).

### Утверждение 2: Биогидрогенизация в рубце уничтожает 60–90% полиненасыщенных ЖК

Линолевая кислота (C18:2) в среднем на 80% биогидрогенизируется в рубце; линоленовая (C18:3) — на 92%. Это снижает эффективность передачи незаменимых ЖК в молоко и ограничивает возможность обогащения молока ПНЖК (NASEM 2021, p. 42).

**Доказательства:** Обзоры Jenkins & Bridges (2007), Glasser et al. (2008b).

**Уверенность:** 0,88 (обзорные данные Jenkins & Bridges, 2007; Glasser et al., 2008b, n > 50).

### Утверждение 3: Добавление C16:0 повышает выход молочного жира без депрессии синтеза de novo

Пальмитиновая кислота увеличивает секрецию C16:0 в молоко с предельной эффективностью 15–35%. В отличие от ненасыщенных C18, C16:0 не образует bioactive FAs, подавляющих синтез короткоцепочечных ЖК в молочной железе (NASEM 2021, p. 47).

**Доказательства:** Lock et al. (2013), de Souza et al. (2016), Dorea & Armentano (2017).

**Уверенность:** 0,85 (Lock et al., 2013; de Souza et al., 2016; Dorea & Armentano, 2017, n = 12).

### Утверждение 4: Молочножировая депрессия (MFD) вызывается bioactive FAs, образующимися при низком pH рубца

Trans-10, cis-12 CLA и trans-10 C18:1 напрямую подавляют синтез молочного жира. Условия для их образования: высокая ферментируемость крахмала + низкий руминальный pH + источник ненасыщенных ЖК (NASEM 2021, p. 46).

**Доказательства:** Griinari et al. (1998), Bauman et al. (2011), Matamoros et al. (2020).

**Уверенность:** 0,87 (Griinari et al., 1998; Bauman et al., 2011; Matamoros et al., 2020, мета-анализ).

### Утверждение 5: Оптимальное содержание ЖК в рационе — 3–5% DM, максимум 7% DM

Рационы с >7% ЖК снижают потребление сухого вещества и могут вызывать молочножировую депрессию. В ранней лактации рекомендуется ограничение <5% DM из-за риска снижения аппетита (NASEM 2021, p. 48).

**Доказательства:** Обзор Chilliard (1993), Weld & Armentano (2017), NRC (2001).

**Уверенность:** 0,82 (Chilliard, 1993; Weld & Armentano, 2017; NRC 2001, обзорные данные).
...
### 4.1. Метаболизм жиров в пищеварительном тракте — Физиология и механизмы
...
**Ключевые процессы (NASEM 2021, p. 42):**
...
**Обоснование.** Модель NASEM 2021 предполагает, что глицерол, образующийся при гидролизе триглицеридов, эквивалентен углеводу и ферментируется в рубце в пропионат и бутират. Это упрощение игнорирует возможность прямой абсорбции глицерола в тонком кишечнике [вне NASEM 2021: Noble, 1981].

---

### 4.2. Модель переваримости жиров — Модель и уравнения

#### Eq 4-1: Кишечная переваримость ЖК (Boerman et al., 2015a)
...
**Обоснование.** Модель NASEM 2021 предполагает линейное снижение кишечной переваримости с ростом потока ЖК в двенадцатиперстную кишку (NASEM 2021, p. 44). Интерсепт 82,5% соответствует теоретическому максимуму при нулевом потоке; наклон −0,0088 отражает конкуренцию за мицеллярную эмульгацию и абсорбцию.

---

#### Eq 4-2: Концентрация белка в молоке при добавлении жира (Wu & Huber, 1994)

**y = 101,1 − 0,638x + 0,0141x²**

Где y — концентрация белка [(обработанная/контроль, %) × 100]; x — процент общих ЖК в рационе.

**Обоснование.** Уравнение описывает параболическое снижение концентрации белка с увеличением содержания ЖК в рационе. Минимум достигается при x ≈ 22,6% DM, что выше практических уровней (NASEM 2021, p. 45). Следовательно, в диапазоне 3–7% DM снижение концентрации белка умеренное и компенсируется ростом удоя; выход белка обычно сохраняется или возрастает.
...
#### Eq 4-3: Переваримость смешанного рациона (взвешенное среднее)
...
**Обоснование.** Взвешенное среднее предполагает аддитивность переваримости отдельных источников. Модель NASEM 2021 предполагает отсутствие взаимодействий между классами источников, что является упрощением при смешивании жиров с сильно различающимися йодными числами [интерполяция: при смешивании насыщенных и ненасыщенных добавок возможна синергия эмульгации].

---

### 4.3. Кишечная абсорбция жирных кислот — Физиология и механизмы

#### Механизм абсорбции в тонком кишечнике

Абсорбция жирных кислот в тонком кишечнике требует предварительной эмульгации желчными кислотами, гидролиза панкреатической липазой и формирования мицелл (NASEM 2021, p. 44). 85–90% ЖК, покидающих рубец, представлены свободными жирными кислотами; 10–15% — микробиальными фосфолипидами.

**Обоснование.** Кишечная переваримость линейно снижается с ростом потока ЖК в двенадцатиперстную кишку согласно Eq 4-1. При потоке 100 г/сут переваримость составляет 81%, при 1800 г/сут — 67% (NASEM 2021, p. 44). Снижение обусловлено ограниченностью поверхности абсорбции и концентрации желчных кислот.

#### Эволюция модели по сравнению с NRC 2001

| Аспект | NRC 2001 | NASEM 2021 | Обоснование |
|--------|----------|------------|-------------|
| Модель переваримости | Единая (0,77) | Классовая (9 категорий) | Daley et al., 2020 |
| Эндогенные фекальные ЖК | Не учтены | Установлены в 0 | Упрощение модели |
| Зависимость от йодного числа | Упоминалась | Явная (через классы) | Firkins & Eastridge, 1994 |

> **FPF A.7 Strict Distinction:** Модель NASEM 2021 предполагает, что кишечная переваримость определяется исключительно потоком ЖК в двенадцатиперстную кишку. Реальная абсорбция зависит также от активности панкреатической липазы, концентрации желчных кислот и pH тонкого кишечника [вне NASEM 2021: Noble, 1981].

---

### 4.4. Молочножировой синтез и депрессия — Физиология и механизмы

#### Механизм синтеза молочного жира

Молочный жир синтезируется в альвеолярных клетках молочной железы из двух источников: синтеза de novo (C4:0–C16:0) из ацетата и β-гидроксимасляной кислоты и инкорпорации поглощённых длинноцепочечных ЖК (NASEM 2021, p. 46).

**Обоснование.** Пальмитиновая кислота (C16:0) увеличивает секрецию жира в молоко с предельной эффективностью 15–35%, не образуя bioactive FAs и не подавляя синтез de novo (NASEM 2021, p. 47).

#### Механизм молочножировой депрессии

Молочножировая депрессия (MFD) вызывается bioactive FAs, в частности trans-10, cis-12 CLA и trans-10 C18:1, образующимися при низком pH рубца и высоком содержании ненасыщенных ЖК (NASEM 2021, p. 46).

**Обоснование.** Указанные изомеры напрямую подавляют экспрессию генов липогенеза в молочной железе, включая ацетил-КоА-карбоксилазу и жирнокислотсинтетазу (Bauman et al., 2011) [вне NASEM 2021: Bauman et al., 2011].

#### Эволюция модели по сравнению с NRC 2001

| Аспект | NRC 2001 | NASEM 2021 | Обоснование |
|--------|----------|------------|-------------|
| Модель MFD | Описана качественно | Количественные оценки (60–90% ПНЖК) | Matamoros et al., 2020 |
| Bioactive FAs | Упоминались | Детальный анализ механизмов | Griinari et al., 1998; Bauman et al., 2011 |
| Рекомендации по ЖК | <7% DM | <7% DM (<5% в ранней лактации) | Weld & Armentano, 2017 |

> **FPF A.6.3 ConservativeRetextualization:** Описание механизмов MFD основано на данных Griinari et al. (1998) и Bauman et al. (2011) [вне NASEM 2021]. NASEM 2021 интегрирует эти данные в практические рекомендации (NASEM 2021, p. 46–47).

---

### 4.5. Энергетическая ценность и практическое применение — Физиология и механизмы

#### Физиологические основы ограничения содержания жира

Содержание жира в рационе ограничено 7% DM из-за риска снижения поедаемости сухого вещества, нарушения пищеварения и молочножировой депрессии (NASEM 2021, p. 48).

**Обоснование.** В ранней лактации рекомендуется ограничение <5% DM, поскольку высокие уровни жира снижают аппетит и замедляют адаптацию рубцовой микробиоты к рациону (NASEM 2021, p. 48).

> **FPF A.7 Strict Distinction:** Модель NASEM 2021 предполагает, что оптимальное содержание ЖК составляет 3–5% DM. Реальная толерантность зависит от физической формы жира, скорости адаптации микробиоты и индивидуальной вариабельности [вне NASEM 2021: Chilliard, 1993].

> **Модель ≠ Реальность:** NASEM 2021 использует фиксированные коэффициенты переваримости, игнорируя влияние количества добавки (Table 4-2). При концентрациях >5% DM модель может переоценивать переваримость.
...
### Q1: Почему недопустимо использовать единый коэффициент 0,77 для всех жиров?
...
|-----|--------|------------|
| Структура и скриншоты | ✅ | Table 4-1, Table 4-2, Figure 4-1, Figure 4-2 |
| Уравнения и расчёты | ✅ | Eq 4-1, Eq 4-2, Eq 4-3 |
| FPF-compliance | ✅ | Числовая шкала уверенности, origin labels |
| Expanded format | ✅ | Физиология, механизмы, эволюция модели |
...
| 2026-05-14 | v1.1 | Kimi Code | Expanded format: физиологические разделы, эволюция модели, FPF-исправления, origin labels, sota_edition, freshness_window, Work Record |
