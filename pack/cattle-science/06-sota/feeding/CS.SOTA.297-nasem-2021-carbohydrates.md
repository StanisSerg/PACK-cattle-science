...
**Уверенность:** средняя–высокая (модель новая, недостаточно полевой валидации).
...
#### Equation 5-1: Расчёт NFC (не рекомендуется для формулирования)

**NFC, % DM = 100 − (NDF + CP + CF + ash)**

**Обоснование.** Eq 5-1 рассчитывает NFC по разности (100 − известные компоненты), что кумулирует аналитические ошибки всех определений. При погрешности NDF ±1,5 %, CP ±0,5 %, ash ±0,3 % и CF ±0,2 % суммарная ошибка NFC достигает ±2,5 % DM. Кроме того, NDSC включает крахмал, сахара, пектины и β-глюканы — компоненты с radically different ферментационными профилями, которые нельзя суммировать в один показатель (NASEM 2021, p. 58; Hall et al., 1999).

---

#### Equation 5-2: Расчёт peNDF (Mertens, 1997)

**peNDF = proportion of particles >1,18 mm × NDF concentration**

Или в модификации Zebeli et al. (2012):

**peNDF >8 = proportion of particles >8 mm × NDF**

**Обоснование.** Eq 5-2 (peNDF) основана на гипотезе, что частицы > 1,18 мм (или > 8 мм в модификации Zebeli) стимулируют жвачку и продукцию слюны — основного буфера руминального pH. Мета-анализ Zebeli et al. (2012) показал breakpoint при 18,5 % peNDF >8 мм (R² = 0,42, RMSE = 0,18 ед. pH): ниже этого порога pH падает, выше — DMI снижается из-за физического наполнения рубца. Коэффициент 1,18 мм выбран Mertens (1997) как порог, выше которого частицы стимулируют жвачку; 8 мм — порог, связанный с pH (NASEM 2021, p. 60).
