...
**Уверенность:** 0,92 (мета-анализ, n = 26 исследований, 129 средних; прямые химические измерения EBW)
**Цитата:** "de Souza and VandeHaar (2018) showed that the equations of the seventh edition generally underestimated the fat content of the empty body and the RE per kilogram of gain in young heifers and overestimated... in older heifers" (NASEM 2021, p. 257)
...
**Уверенность:** 0,88 (мета-анализ, n = 26; линейная модель не уступает квадратичной по AIC/BIC)
...
**Уверенность:** 0,75 (адаптировано от NASEM 2016 beef; прямых измерений на молочных телках недостаточно)
...
**Уверенность:** 0,85 (экспонента 0,097; данные Radcliff et al., 1997 и Waldo et al., 1997)
...
Глава 11 описывает требования к энергии и протеину для растущих молочных телок (replacement heifers) (NASEM 2021, p. 255). Это фундаментальный раздел, обеспечивающий расчёт рационов для телок от отъёма до первого отёла.
...
Связь с другими главами (NASEM 2021, p. 255):
...
> **FPF A.7 Strict Distinction:** Уравнения Garrett описывают **говядину** (object), но были применены к **молочным телкам** (другой object) в NRC 2001. Это category error, выявленная только после накопления данных по молочным породам.
...
#### Эволюция модели: от говяжьих данных к хольштейновской специфике
...
#### Эволюция модели: ключевые параметры

| Параметр | NRC 2001 | NASEM 2021 | Обоснование изменения |
|----------|----------|------------|----------------------|
| База данных | Говяжьи данные (Garrett 1980) | Хольштейны (26 исследований) | Мета-анализ de Souza & VandeHaar (2018) |
| Функция состава | Степень 0,75 BW | Линейная от EBW | Лучшее описание крайних точек (NASEM 2021, p. 258) |
| Жир при BCS=3 | 25% EBW | 22% EBW | Прямые измерения, не предположение |
| MP→NP эффективность | 0,77→0,39 (нелинейная) | 0,64→0,39 (линейная) | Данные не подтвердили нелинейность |
...
| Зависимость от ADG | RE ∝ ADG^1,097 (NRC 2001) | RE ∝ ADG^0,097 (NASEM 2021) | Рационы менее чувствительны к скорости роста |
...
Модель требований для роста телок состоит из трёх компонентов (NASEM 2021, p. 258):
...
```
EBW = 0.85 × BW                          (Eq 11-1a; NASEM 2021, p. 258)
EBG = 0.85 × ADG                         (Eq 11-1b; NASEM 2021, p. 258)
```
...
```
ME maintenance (Mcal/d) = 0.15 × BW^0.75   (Eq 11-2; NASEM 2021, p. 258)
```

MP maintenance:
```
MP-scurf (г/д) = (0.20 × BW^0.60) / 0.69          (Eq 11-3a; NASEM 2021, p. 258)
MP-endogenous urinary (г/д) = 53 × 6.25 × BW × 0.001 (Eq 11-3b; NASEM 2021, p. 258)
MP-MFP (г/д) = ((11.62 + 0.134 × NDF% сухого вещества) × DMI) / 0.69 (Eq 11-3c; NASEM 2021, p. 258)
```
...
```
Fat in EBW (кг/кг) = 0.067 + 0.375 × (BW / MatBW)     (Eq 11-4a; NASEM 2021, p. 258)
FFM in EBW (кг/кг) = 1 - Fat_EBW                      (Eq 11-4b; NASEM 2021, p. 258)
Protein in EBW (кг/кг) = 0.215 × FFM_EBW              (Eq 11-4c; NASEM 2021, p. 258)
Ash in EBW (кг/кг) = 0.056 × FFM_EBW                  (Eq 11-4d; NASEM 2021, p. 258)
Water in EBW (кг/кг) = 0.729 × FFM_EBW                (Eq 11-4e; NASEM 2021, p. 258)
```
...
```
Fat in EBG (кг/кг) = 0.067 + 0.375 × (BW / MatBW)     (Eq 11-5a; NASEM 2021, p. 258)
FFM in EBG (кг/кг) = 1 - Fat_EBG                       (Eq 11-5b; NASEM 2021, p. 258)
Protein in EBG (кг/кг) = 0.215 × FFM_EBG               (Eq 11-5c; NASEM 2021, p. 258)
RE in EBG (Mcal/кг) = 9.4 × Fat_EBG + 5.55 × Protein_EBG (Eq 11-5d; NASEM 2021, p. 258)
Ash in EBG (кг/кг) = 0.056 × FFM_EBG                   (Eq 11-5e; NASEM 2021, p. 258)
Water in EBG (кг/кг) = 0.729 × FFM_EBG                 (Eq 11-5f; NASEM 2021, p. 258)
```
...
```
Fat in ADG (кг/кг) = 0.85 × Fat_EBG                    (Eq 11-6a; NASEM 2021, p. 259)
RE in ADG (Mcal/кг) = 0.85 × RE_EBG                    (Eq 11-6b; NASEM 2021, p. 259)
Protein in ADG (кг/кг) = 0.85 × Protein_EBG            (Eq 11-6c; NASEM 2021, p. 259)
```

#### Уравнения требований к росту

```
ME for growth (Mcal/д) = RE (Mcal/д) / 0.40            (Eq 11-7; NASEM 2021, p. 259)
```

Эффективность MP→NP:
```
NP-eff = 0.64 - 0.30 × (EBW / Mature EBW)             (Eq 11-8; NASEM 2021, p. 259)
MP for growth (г MP/д) = Retained protein (г/д) / NP-eff (Eq 11-9; NASEM 2021, p. 259)
```

#### Уравнения минимального соотношения протеин:энергия

```
Minimum MP (г/Mcal ME) = 53 - 25 × (BW / MatBW)     (Eq 11-10; NASEM 2021, p. 260)
```

Корректировка MP при недостаточном соотношении:
```
If MP (г/д) < (53 - 25 × BW/MatBW) × ME (Mcal/д):
    MP (г/д) = (53 - 25 × BW/MatBW) × ME (Mcal/д)   (Eq 11-11; NASEM 2021, p. 260)
```
...
**Описание:**
Требование ME для поддержания растущих телок установлено как 0,15 × BW^0,75 Мcal/д (NASEM 2021, p. 258). Это соответствует требованию для взрослых коров и близко к значению NASEM 2016 для говяжьих пород (0,095 × BW^0,75 на NE-базе).
...
Модель NASEM 2021 предполагает, что коэффициент 0,15 × BW^0,75 отражает энергетические затраты на поддержание базового обмена веществ и терморегуляцию у растущих телок (NASEM 2021, p. 258). Экспонента 0,75 характерна для аллометрического масштабирования метаболизма млекопитающих. Стандартная корректировка на активность +10% учитывает минимальное перемещение в stall-fed условиях; для пастбищного содержания рекомендуется дополнительное увеличение на 15–20% [guess: данные по пастбищной активности для телок ограничены].

#### Механизм поддержания

**Базальный обмен.** Основные энергетические затраты на поддержание включают синтез и деградацию белков, поддержание ионных градиентов (Na⁺/K⁺-АТФаза), работу сердца и дыхания. Масштабирование по BW^0,75 отражает отношение поверхности к объёму и эффективность энергетического обмена (NASEM 2021, p. 258).

**Терморегуляция.** При температуре ниже нижней критической точки (LCT ~10–15°C для телок) затраты на терморегуляцию увеличиваются пропорционально разнице температур. NASEM 2021 не содержит корректировки на холодный климат; при содержании без утепления ниже −10°C рекомендуется увеличивать maintenance на 10–20% [вне NASEM 2021: NRC, 2001; Fox et al., 2004].

---

### 5.2. Состав прироста по стадиям роста — Физиология и механизмы

**Соответствует:** Figure 11-2, Equations 11-4 — 11-6

**Описание:**
Состав прироста (ADG) меняется от преимущественно протеина и воды в молодом возрасте до преимущественно жира при приближении к зрелости (NASEM 2021, p. 258–259).
...
**Модель NASEM 2021** предполагает, что содержание жира в EBW линеаризуется относительно отношения BW/MatBW (Eq 11-4a). Это противоположно степенной модели NRC 2001 (BW^0,75) и отражает физиологический факт: молочные породы откладывают жир позже и медленнее, чем мясные (NASEM 2021, p. 258). Линейная функция лучше предсказывает состав тела на крайних стадиях роста — у молодых телок и у взрослых коров.

#### Механизм композиционных изменений

**Мышечный рост.** В молодом возрасте преобладает гиперплазия и гипертрофия миофибрилл. Содержание протеина в EBW составляет ~19–20%, вода ассоциирована с белком в соотношении ~3,4:1 (water:protein), что определяет высокую долю безжировой массы (FFM) в приросте (NASEM 2021, p. 258).

**Жиронакопление.** По мере приближения к зрелости доля жира в EBW увеличивается от ~10% до ~22%. Этот процесс регулируется гормональным статусом (инсулин, соматотропин, эстрогены) и энергетическим балансом. Модель NASEM 2021 не учитывает текущий BCS; она предполагает оптимальное состояние BCS = 3 (NASEM 2021, p. 259).

---

### 5.3. Эффективность MP→NP — Физиология и механизмы

#### Обоснование снижения эффективности

**Модель NASEM 2021** предполагает, что эффективность конверсии MP→NP снижается линейно от 0,64 до 0,39 (Eq 11-8). Это отражает биохимический факт: с возрастом снижается способность к белковому синтезу de novo, а аминокислоты направляются преимущественно на поддержание, а не на аккрецию (NASEM 2021, p. 259).

#### Механизм белкового синтеза

**Регуляция на уровне трансляции.** Эффективность использования аминокислот зависит от концентрации инсулиноподобного фактора роста-1 (IGF-1), доступности энергии (ATP для синтеза и аминоацил-тРНК) и гормонального фона. У молодых телок высокий уровень соматотропина и IGF-1 стимулирует синтез мышечного протеина; у старших телок анаболический тон снижается (NASEM 2021, p. 259).

**Практическая импликация.** Снижение NP-eff с 0,64 до 0,39 означает, что для синтеза 1 г нетто-протеина старшей телке требуется в 1,64 раза больше MP, чем молодой. Это объясняет необходимость разделения телок на возрастные группы при групповом кормлении (NASEM 2021, p. 260).

**Соответствует:** Equation 11-8

**Описание:**
Эффективность конверсии MP в нетто-протеин снижается с возрастом от 0,64 до 0,39 (NASEM 2021, p. 259).
...
### 5.4. Соотношение протеин:энергия (MP/ME и CP/ME) — Физиология и механизмы

#### Обоснование минимального MP/ME

**Модель NASEM 2021** предполагает, что минимальное соотношение MP/ME защищает от избыточного жиронакопления при недостатке протеина (NASEM 2021, p. 260). Это основано на биохимическом механизме: при ограничении аминокислот организм направляет избыток энергии преимущественно в липогенез, а не в мышечный синтез.

#### Механизм регуляции липогенеза

**Физиологическая основа.** При адекватном протеиновом статусе инсулин и аминокислоты активируют мТОР-путь в мышцах, стимулируя синтез белка. При дефиците протеина анаболический сигнал снижается, а доступная энергия направляется в адипоциты для триглицеридного синтеза [вне NASEM 2021: Suryawan et al., 2006].

**Молекулярный уровень.** AMPK (AMP-активированная протеинкиназа) действует как сенсор энергетического статуса клетки. При высоком AMP/ATP отношении AMPK ингибирует анаболические пути (включая белковый синтез через ингибирование mTORC1) и активирует катаболизм. Этот механизм объясняет, почему недостаток протеина в сочетании с избытком энергии приводит к преимущественному жиронакоплению (NASEM 2021, p. 260).
...
**Описание:**
Это ключевой параметр для практики. NASEM 2021 ввёл **минимальное соотношение MP/ME**, которое определяет требование к протеину в рационе (NASEM 2021, p. 260). Если расчётное MP из прироста ниже этого порога — используется пороговое значение.
...
**Вывод комитета:** Диеты с избытком энергии и недостатком протеина наиболее вредны для молочной железы. Минимальное MP/ME = **53 - 25 × BW/MatBW** защищает от этого (NASEM 2021, p. 260).

> **FPF A.7 Strict Distinction:** Модель NASEM 2021 устанавливает минимальное MP/ME как пороговое значение, но не моделирует молекулярные механизмы липогенеза. Описанные биохимические пути (mTOR, AMPK) приведены для интерпретации [вне NASEM 2021: Suryawan et al., 2006].
...
**Модель NASEM 2021:** Компенсаторный рост **не моделируется**. **Модель NASEM 2021** предполагает, что состав прироста зависит только от BW/MatBW, а не от истории роста (NASEM 2021, p. 260).

**Данные:** Harrison et al. (1983) и Park et al. (1987) показали, что при компенсаторном росте прирост содержит **больше жира и меньше протеина**, чем при нормальном росте той же массы [вне NASEM 2021: Harrison, 1983; Park, 1987]. Это объясняется тем, что организм направляет избыток энергии преимущественно в жировые депо, а не в мышечный синтез.
...
> **FPF A.10 Evidence:** Отсутствие модели компенсаторного роста в NASEM 2021 — **guess** на уровне предположения, основанного на ограниченном числе исследований [вне NASEM 2021: Harrison, 1983; Park, 1987].
...
> **FPF A.10 Evidence:** Данных по кроссбредным телкам в мета-анализе NASEM 2021 практически нет. Рекомендации основаны на предположениях (guess) и требуют валидации на конкретных стадах.
...
> **FPF B.3 F-G-R:** Данные по ADG > 1,2 кг/д для молочных телок ограничены. Рекомендации для экстремальных скоростей роста — guess.
...
| Зависимость от ADG | RE ∝ ADG^1,097 | RE ∝ ADG^0,097 | Рацион менее чувствителен к скорости роста |
...
> **FPF A.7 Strict Distinction:** Следующая оценка относится к модели NASEM 2021 как к описательной системе, а не к биологической реальности роста телок.
...
- **Gut fill:** Принят константой 15%, хотя реальные данные показывают 11-19% в зависимости от рациона (NASEM 2021, p. 258). Комитет признаёт необходимость дальнейшей работы.
- **Активность:** Только +10%, без корректировки на пастбищное содержание, компенсаторный рост, температуру (NASEM 2021, p. 258).
- **MatBW:** Требует точной оценки; для кроссбредов и других пород может отличаться (NASEM 2021, p. 261).
- **BCS:** Нет корректировки на текущий BCS; **модель NASEM 2021** предполагает оптимальное состояние (NASEM 2021, p. 259).
...
| Дата | Действие | Исполнитель | Статус |
|------|----------|-------------|--------|
| 2026-05-10 | Создание SoTA из главы 11 NASEM 2021 | Стратег (R1) | ✅ Выполнено |
| 2026-05-10 | Извлечение текста из PDF, структурирование уравнений | Стратег (R1) | ✅ Выполнено |
| 2026-05-10 | Добавление медиа-инвентаря, FAQ, практического применения | Стратег (R1) | ✅ Выполнено |
| 2026-05-14 | Приведение в соответствие с SOTA-CHAPTER-EXPANDED-TEMPLATE.md v1.3 | Стратег (R1) | ✅ Выполнено |

### 13.2. Work Record

| Дата | Действие | Результат | Версия |
|------|----------|-----------|--------|
| 2026-05-10 | Создание SoTA из главы 11 NASEM 2021 | Полная структура, уравнения, медиа | v1.0 |
| 2026-05-10 | Добавление секции 5.4 (протеин:энергия), Equations 11-10/11-11, Tables 11-3/11-4, обновление FAQ | Расширенное покрытие требований | v1.1 |
| 2026-05-10 | Вставка скриншотов Table 11-3 и Table 11-4 как PNG изображения | 6 PNG файлов | v1.2 |
| 2026-05-10 | Автоматическая обрезка PNG: PyMuPDF clip по bounding box таблиц/графиков | Экономия ~62%, 3.3→1.24 MB | v1.3 |
| 2026-05-10 | Переобрезка таблиц: включены footnotes/captions | Table 11-1: Heifers+Calves+Cows; Table 11-3/11-4: примечания | v1.4 |
| 2026-05-10 | Умная обрезка: автоопределение описаний с FIGURE/TABLE после таблиц | Table 11-1: +описание до Eq 11-4a | v1.5 |
| 2026-05-10 | Финальная обрезка по примеру: таблицы=footnote-only, графики=image+caption | gap threshold 3pt | v1.6 |
| 2026-05-14 | Приведение в соответствие с Expanded Template v1.3 | Исправлены 6 FPF-ошибок, добавлены 3 физиологических раздела, эволюция модели, origin-метки, Work Record | v1.7 |

**Следующие шаги:**
- [x] Добавить скриншоты Figure 11-1, 11-2, Table 11-1
- [x] Привести в соответствие с SOTA-CHAPTER-EXPANDED-TEMPLATE.md v1.3
