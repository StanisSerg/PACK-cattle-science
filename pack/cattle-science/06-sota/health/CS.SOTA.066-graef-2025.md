...
| Table 1 | Таблица | Средний удой при разных порогах tCa в 5 DIM | Нет (текстовая) | ⏭️ Пропущено — числа в тексте |
| Table 2 | Таблица | Дескриптивная статистика по группам | Нет (текстовая) | ⏭️ Пропущено — числа в тексте |
| Table 3 | Таблица | DMI, MY, ECM, FCM (LSM, 95% ДИ) | Нет (текстовая) | ⏭️ Пропущено — числа в тексте |
| Table 4 | Таблица | Концентрации tCa, P, Mg (LSM, 95% ДИ) | Нет (текстовая) | ⏭️ Пропущено — числа в тексте |
| Table 5 | Таблица | Hp, SAA, TNFα, IFNγ, IL-10 (LSM, 95% ДИ) | Нет (текстовая) | ⏭️ Пропущено — числа в тексте |

> **Примечание:** Все значимые фигуры извлечены как PNG (200 dpi). Текстовые таблицы воспроизведены в разделе 5 в виде markdown-таблиц. Мусорные auto-page PNG удалены.
