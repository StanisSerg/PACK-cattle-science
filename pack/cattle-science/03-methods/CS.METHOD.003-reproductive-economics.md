...
status: active
sota: 2025-03-01
sources: [Lauber-2025, Cabrera-2012, Giordano-2012, Hansson-2025]
language: ru
created: 2026-03-01
updated: 2026-03-03
---
...
3. Экономической ценности улучшения 21-дневного индекса стельности (21-d PR)
...
|----------|-------------|----------|---------|-----------|
| 21-дневный индекс стельности | 21-d PR | 20-40 | % | (SR × P/AI) × 100 |
...
| Рыночная цена мясных телят | — | 385-1 125 | $/телёнок | За голову |
...
### Референсные значения: 21-дневный индекс стельности (21-d PR)
...
**Компоненты расчёта (референсные значения Lauber 2025):**

| Компонент | Формула / Значение | Примечание |
|-----------|-------------------|------------|
| **Доход от молока** | Удой (кг) × Цена молока ($/кг) | Зависит от дней в молоке |
| **Доход от телят Conv** | 0.5 × (Дойные + Неплодные) × $200 | 50% телок, цена $200/голова |
| **Доход от телят Beef×Dairy** | %Beef × (Дойные + Неплодные) × $700 | %Beef = доля мясной спермы, цена $700/голова |
| **Затраты на сперму Conv** | Кол-во осеменений × $10 | ~$10/доза |
| **Затраты на сперму Sexed** | Кол-во осеменений × $20 | ~$20/доза |
| **Затраты на сперму Beef** | Кол-во осеменений × $5 | ~$5/доза |
| **Затраты на замену** | Норма выбытия × Стоимость нетели | ~$1500/нетель |
| **Прочие затраты** | Фиксировано или % от дохода | Зависит от фермы |

**Упрощённый расчёт (для быстрой оценки):**
- Базовый сценарий (20% PR, Conv, 170d IEP): NR = 0 (нормировано)
- Sexed+Beef vs Conv: +$51/корова/год
- Каждый +1% к PR: +$2-7/корова/год

**Референсные значения (Lauber 2025, для 170-дневного IEP):**
- Базовая NR (20% 21-d PR, Conv): нормирована к 0 (базовый уровень для сравнения)
- Улучшение NR (Sexed+Beef vs Conv при 20% PR): +$51/корова/год
- Прирост NR на 1% улучшения 21-d PR: $2-7/корова/год (зависит от типа спермы)

### Этап 4: Определение оптимального IEP

Найти IEP, максимизирующий NR при достаточности замены:

| 21-d PR | Оптимальный IEP (Conv) | Оптимальный IEP (Sexed+Beef) | Расширенный VWP* |
|---------|------------------------|------------------------------|------------------|
| 20% (Низкая) | 170 дней | 200 дней** | — |
| 25% (Ниже среднего) | 150 дней | 200 дней** | — |
| 30% (Средняя) | 130 дней | 200 дней | 140 дней*** |
| 35% (Выше среднего) | 100 дней | 200 дней | 140 дней*** |
| 40% (Высокая) | 80 дней | 200 дней | 140 дней*** |

*VWP = Voluntary Waiting Period (период ожидания добровольный) — время от отёла до первого осеменения; IEP в таблице = время от конца VWP до последнего осеменения  
**Требуется HSR ≥80% для досточности замены при стратегии Sexed+Beef  
***Расширенный VWP (140 дней): для second-parity cows из high-producing стад (Hansson 2025) — альтернатива короткому VWP, не связанная напрямую с IEP

### Этап 5: Рассмотреть расширенный VWP (опционально)

Для **second-parity cows** из **high-producing стад** (Hansson 2025):
...
- [ ] Parity = 2 (вторая лактация)
- [ ] High-producing herd (≥10,000 кг молока)
- [ ] Готовность к длинному calving interval

**Рекомендация:** VWP 140 дней — жизнеспособная альтернатива для second-parity cows.
...
*Ревизия: 2026-03-03 — исправлены нумерация этапов, формулы, уточнены VWP/IEP*
