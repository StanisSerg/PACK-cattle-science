...
|--------|----------|-------|
| [Полный список](#полный-список-sota) | Все 63 источников с тегами | — |
...
### Кормление и метаболизм (14)

| ID | Статья | Год | Тип | Теги | Ключевой результат |
|----|--------|-----|-----|------|-------------------|
| CS.SOTA.064 | Bruner et al. | 2025 | Experimental | `#анионные-соли` `#DCAD` `#предпочтение` `#потребление` | Микроинкапсулированные анионные соли — высокое потребление |
...
### Здоровье и метаболизм (32)

| ID | Статья | Год | Тип | Теги | Ключевой результат |
|----|--------|-----|-----|------|-------------------|
| CS.SOTA.065 | Ghaffari et al. | 2024 | Experimental | `#метаболомика` `#печень` `#ацилкарнитины` `#митохондрии` | Неполное β-окисление ЖК в ранней лактации — ключевой механизм |
| CS.SOTA.066 | Graef et al. | 2025 | Field-study | `#гипокальциемия` `#воспаление` `#SAA` `#TNFα` | 4 фенотипа SCH с разными профилями воспаления |
| CS.SOTA.067 | Lin et al. | 2025 | ML-study | `#FTIR` `#machine-learning` `#предсказание` `#ранняя-лактация` | PLS-DA предсказывает заболевания за 7 дней (AUROC 0.71) |
| CS.SOTA.068 | Caixeta & Omontese | 2021 | Review | `#мониторинг` `#менеджмент` `#переходный-период` | Комплексный подход к метаболическому здоровью |
...
| CS.SOTA.058 | Kupczyński et al. | 2020 | Review | `#глицерол` `#кетоз` `#лечение` | Глицерол как лечение кетоза (Animals 1379) |
| CS.SOTA.059 | Kristensen & Raun | 2007 | Field-study | `#пропиленгликоль` `#метаболизм` `#кетоз` | Метаболизм PG: гепатический захват 19%, два механизма |
| CS.SOTA.060 | Capel et al. | 2021 | Field-study | `#пропиленгликоль` `#декстроза` `#гиперкетонемия` | PG ± дексроза для лечения HYK, RCT |
| CS.SOTA.061 | Bjerre-Harpøth et al. | 2015 | Experimental | `#пропиленгликоль` `#жировая-ткань` `#липолиз` | PG снижает мобилизацию жировой ткани у перекормленных коров |
| CS.SOTA.062 | Lomander et al. | 2012 | Field-study | `#глицерол` `#пропиленгликоль` `#сравнение` | Глицерол vs PG в ранней лактации, Швеция |
| CS.SOTA.063 | Kristensen & Raun | 2007 | Experimental | `#пропиленгликоль` `#метаболизм` `#рубец` `#печень` | Метаболизм PG: руминальный и гепатический |
...
| ID | Статья | Год | Тип | Теги | Ключевой результат |
|----|--------|-----|-----|------|-------------------|
| CS.SOTA.026 | Various | 2025 | Framework | `#репродуктивная-оценка` `#алгоритм` `#21-дневная-стельность` | Алгоритм оценки воспроизводства |
