...
**Соответствует:** [Ссылка на медиа-элемент из 4.3, например: Figure 1]

**Описание визуала:**
[Что показано на графике/таблице]
...
**Комментарий лектора:**
> "[Дословная фраза для объяснения этого результата]"

---

## 5.2. [Название результата / Table X]

**Соответствует:** [Ссылка на Table X из 4.3]
...
> **Правило:** Индекс обновляется НЕМЕДЛЕННО после завершения каждой SoTA.
> 
> **Автоматизация:** Если настроены git hooks — индекс обновится автоматически после коммита.

**Ручное обновление (если нет автоматизации):**
```bash
bash scripts/add-sota-to-index.sh pack/cattle-science/06-sota/[area]/CS.SOTA.XXX-*.md
```

**Проверка:**
- [ ] SoTA видна в индексе при поиске Ctrl+F
- [ ] Счётчики обновлены

**Без этого шага SoTA считается НЕЗАВЕРШЁННОЙ.**

### Phase 7: Финал (5 мин)
- [ ] Заполнить журнал обработки
- [ ] Проверить по ArchGate (должно быть ≥8)
- [ ] Закоммитить в git: `git add -A && git commit -m "CS.SOTA.XXX: [краткое описание]"

#### ⚠️ ПРАВИЛО ОБНОВЛЕНИЯ ИНДЕКСА (АВТОМАТИЗИРОВАНО)

> **Автоматизация:** Индекс обновляется автоматически при коммите (через git hooks).
> 
> **Ручной режим:** `bash scripts/add-sota-to-index.sh <файл>`
...
1. После `git commit` hook проверяет новые SoTA-файлы
2. Извлекает метаданные из YAML
3. Добавляет строки в таблицы индекса
4. Обновляет счётчики и дату
5. Добавляет индекс в коммит
