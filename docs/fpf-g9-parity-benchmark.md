...
# G.9 — Parity / Benchmark Harness (Сравнение методов на единой базовой линии)
...
## 5. Пример на ферме: сравнение трёх методов диагностики мастита
...
- **G.9:Ext.DHCParityPins** — сравнение DHC-слотов (C.21) под parity
- **G.9:Ext.CausalMethodRungParity** — parity для каузальных методов (C.28)
- **G.5:PortfolioMode** — как селектор возвращает множества, а не скаляры
- **FPF-Spec.md §G.9** — полная нормативная версия
