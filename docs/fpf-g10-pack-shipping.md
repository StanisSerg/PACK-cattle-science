...
# G.10 — SoTA Pack Shipping (Передача SoTA-упаковки без потери семантики)
...
## 5. Пример на ферме: передача протокола переходного периода
...
- **G.10:Ext.QDArchiveShippingPins** — специфические штифты для QD-отправки
- **G.10:Ext.OEEShippingPins** — штифты для OEE-упаковок
- **G.11 (Telemetry-Driven Refresh)** — как обновлять shipped pack
- **FPF-Spec.md §G.10** — полная нормативная версия
