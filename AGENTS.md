...
| Контроль / разрешения / записи отдела | инструкции D-NN, контур сбора, адм. исключения | ADM.1 (рамка работы), ADM.7 (проверка утверждения), ADM.10 (должны/выполнили/записали), ADM.13 (записи), ADM.14 (контроль→цель→решение), ADM.16 (смена устройства) |
| KPI / цели / учёт ресурсов | KPI-блоки, целевые показатели | MA.6 (цель ≠ прогноз ≠ запрос), MA.7 (объяснение отклонений), MA.9 (поведенческие эффекты показателя) |
